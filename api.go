...
func (c *Client) getJSONWithRetry(
	...
	if e, ok := err.(*types.Error); ok {
		logger.Debug(err.Error(), fmt.Sprintf("Got JSON error: %+v", e), 1)
		if e.HTTPStatusCode == 401 {
			logger.Info("Need to re-auth")
			// Authenticate then try again
			if _, err := c.Authenticate(c.configConnect); err != nil {
...
func NewClientWithArgs(
	...
	logger.Debug(fmt.Sprintf("goscaleio client init, Fields: %+v", fields))

	if endpoint == "" {
		logger.Debug(fmt.Sprintf("endpoint is required, Fields: %+v", fields))
		return nil,
	...
	if err != nil {
		logger.Debug(fmt.Sprintf("Unable to create HTTP client: %s", err.Error()))
		return nil, err
	}
